 ...
 * @Description:
 */
class Client1(serverHost : String, serverPort : Int) extends Actor {
    //定义一个Service的Ref
    ...
    override def receive : Receive = {
        case "start" => println("客户端1 start，可以咨询问题了!")
        case msg : String => {
            //发给服务器（客户）
...
object Client1 extends App {
    ...
    val (clientHost, clientPort, serverHost, serverPort) = ("127.0.0.1", 9990, "127.0.0.1", 19999)
    ...
    private val client1 : ActorRef = clientActorSystem.actorOf(Props(new Client1(serverHost, serverPort)), "xx")
